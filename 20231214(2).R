...
for(i in 1:length(age)){ #估每個年齡層的mu
  mu_tmp1 <- uniroot(f.mu2, assir_m_age=assir_m[i], xb_b1=xb_b1_tmp1, interval=c(-10,10))$root
  mu_1 <- c(mu_1, mu_tmp1)

  tmp_index2 <- intersect(which(data.s$case==0), which(data.s$age==age[i]))
  if(length(tmp_index2) >= 10){ #總共多少人(樣本數夠大則符合條件)
    xb_b1_tmp2 <- x_s[tmp_index2,]%*%b_1 #算出xbeta(=beta2*x2+beta3*x3)
    mu_tmp2 <- uniroot(f.mu2, assir_m.age=assir_m[i], xb_b1=xb_b1_tmp2, interval=c(-10,10))$root
    mu_2 <- c(mu_2, mu_tmp2)
  }else{
    mu_2 <- c(mu_2,NA)
  }
